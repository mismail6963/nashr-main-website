...
export default async function LocaleLayout({
  ...
  return (
    <html lang={locale} dir={dir} className={fontClass}>
      <body>
        {/* On-screen diagnostics (Part A) — flag-gated & removable.
            Enable on an affected device by opening the site with ?debug=1
            (persists for the tab; ?debug=0 turns it off). Disabled => this
            whole block is an immediate no-op for normal visitors.
            It runs inline (not bundled) so it still surfaces failures even when
            the app bundle itself is blocked by a content blocker / ITP / Private
            Relay. It captures uncaught errors, unhandled promise rejections and
            BLOCKED RESOURCES (script/style/img), exposes window.__nashrDiag for
            the React layer, and after load reports whether the app hydrated and
            how many elements are still hidden — pinpointing stuck content. */}
        <script
          dangerouslySetInnerHTML={{
            __html:
              "(function(){var on=false;try{var q=location.search;if(/[?&]debug=1/.test(q)){try{sessionStorage.setItem('nashr_debug','1')}catch(e){}}if(/[?&]debug=0/.test(q)){try{sessionStorage.removeItem('nashr_debug')}catch(e){}}on=/[?&]debug=1/.test(q);try{if(sessionStorage.getItem('nashr_debug')==='1')on=true}catch(e){}}catch(e){}if(!on)return;window.__nashrNoReload=true;var box,list,lines=[];function ensure(){if(box)return;box=document.createElement('div');box.setAttribute('dir','ltr');box.style.cssText='position:fixed;left:8px;right:8px;bottom:8px;z-index:2147483647;max-height:45vh;overflow:auto;background:rgba(18,18,20,.96);color:#f5f5f5;font:11px/1.45 ui-monospace,Menlo,Consolas,monospace;padding:10px 12px;border:1px solid #C7B27A;border-radius:8px;white-space:pre-wrap;word-break:break-word;-webkit-user-select:text;user-select:text';var h=document.createElement('div');h.style.cssText='display:flex;justify-content:space-between;gap:8px;margin-bottom:6px;color:#C7B27A;font-weight:700';var t=document.createElement('span');t.textContent='NASHR diagnostics \\u2014 please screenshot';var x=document.createElement('button');x.textContent='\\u00d7';x.setAttribute('aria-label','close');x.style.cssText='background:transparent;border:0;color:#C7B27A;font-size:16px;line-height:1;cursor:pointer';x.onclick=function(){if(box&&box.parentNode)box.parentNode.removeChild(box);box=null};h.appendChild(t);h.appendChild(x);list=document.createElement('div');box.appendChild(h);box.appendChild(list);(document.body||document.documentElement).appendChild(box)}function render(){if(list)list.textContent=lines.slice(-40).join('\\n')}function add(k,m){lines.push('['+k+'] '+m);if(!document.body){document.addEventListener('DOMContentLoaded',function(){ensure();render()});return}ensure();render()}window.__nashrDiag=function(k,m){try{add(k||'app',String(m))}catch(e){}};window.addEventListener('error',function(e){var g=e&&e.target;if(g&&g!==window&&(g.tagName==='SCRIPT'||g.tagName==='LINK'||g.tagName==='IMG')){add('resource-blocked',g.tagName+' '+(g.src||g.href||''));return}add('error',((e&&e.message)||'')+' @ '+((e&&e.filename)||'')+':'+((e&&e.lineno)||''))},true);window.addEventListener('unhandledrejection',function(e){var r=e&&e.reason;add('promise',(r&&(r.stack||r.message))||String(r))});function selfCheck(){try{var n=0,a=document.querySelectorAll('[style]');for(var i=0;i<a.length;i++){if(a[i].style.opacity==='0')n++}add('status','hydrated='+(!!window.__appHydrated)+' stillHidden='+n);add('ua',navigator.userAgent)}catch(e){add('status','selfcheck failed: '+e)}}if(document.readyState==='complete')setTimeout(selfCheck,6000);else window.addEventListener('load',function(){setTimeout(selfCheck,6000)});add('init','diagnostics active')})();",
          }}
        />
